!macro NSIS_HOOK_POSTINSTALL
  nsExec::ExecToLog 'powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$INSTDIR\installers\windows\install-service.ps1" -SupervisorExecutable "$INSTDIR\service\supervisor.exe" -DataDirectory "$PROGRAMDATA\LocalAIImageFilter"'
  Pop $0
  ${If} $0 != 0
...
!macro NSIS_HOOK_PREUNINSTALL
  ...
  ${Else}
    ...
    ${EndIf}
    nsExec::ExecToLog 'powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$INSTDIR\installers\windows\uninstall-service.ps1" -AuthorizationFile "$PROGRAMDATA\LocalAIImageFilter\uninstall-authorization.token" -FilterCtlExecutable "$INSTDIR\service\filterctl.exe"'
    Pop $0
    ${If} $0 != 0
